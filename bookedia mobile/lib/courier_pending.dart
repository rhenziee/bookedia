...
class PendingPage extends StatefulWidget {
  final int userId;
  const PendingPage({super.key, required this.userId});
  ...
  Future<void> fetchPendingOrders() async {
    try {
      final response = await http.get(
        Uri.parse('http://192.168.68.116:5000/api/toship'),
      );
              ...
              child: ElevatedButton(
                onPressed: () async {
                  final response = await http.post(
                    Uri.parse(
                      'http://192.168.68.116:5000/api/mark_out_for_delivery/${order.parcelId}',
                    ),
                  );
                  ...
                  if (response.statusCode == 200) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('Marked as out for delivery'),
                      ),
                    );
                    // Optionally refresh the list after marking
                    // ignore: use_build_context_synchronously
                    if (context.findAncestorStateOfType<_PendingPageState>() !=
                        null) {
                      // ignore: use_build_context_synchronously
                      context
                          .findAncestorStateOfType<_PendingPageState>()!
                          .fetchPendingOrders();
                    }
                  } else {
                    // ignore: use_build_context_synchronously
